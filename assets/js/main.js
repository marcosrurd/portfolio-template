...
function initIntroAnimation() {
    ...
    // Crear pre-intro din�micamente (varias l�neas con "portfolio")
    const preIntro = document.createElement('div');
    preIntro.className = 'pre-intro';
    preIntro.setAttribute('aria-hidden', 'true');

    // N�mero de l�neas (ajusta si quieres m�s/menos)
    const lines = 12;
    for (let i = 0; i < lines; i++) {
        const w = document.createElement('div');
        w.className = 'pre-word';
        w.textContent = 'portfolio';
        preIntro.appendChild(w);
    }

    // Insertar la pre-intro antes del t�tulo (cubre la pantalla)
    intro.insertBefore(preIntro, title);

    const introTl = gsap.timeline({
        onComplete: () => {
            intro.remove();
            initHeroAnimations(); // Arranca el hero DESPU�S de la intro
        },
    });

    // Secuencia:
    // 1) Entradas en cascada de las palabras "portfolio"
    // 2) Breve pausa
    // 3) Salida en cascada hacia arriba (desaparecen)
    // 4) Reutilizamos la animaci�n original del t�tulo 'urdiales'
    introTl
        .to(preIntro.querySelectorAll('.pre-word'), {
            opacity: 1,
            y: 0,
            scale: 1,
            stagger: 0.05,
        ...
        .to({}, { duration: 0.6 }) // pausa corta para que el conjunto permanezca visible
        .to(preIntro.querySelectorAll('.pre-word'), {
            opacity: 0,
            y: -40,
            stagger: 0.03,
        ...
        .add(() => {
            // Despu�s de ocultar preIntro, mostramos el t�tulo "urdiales"
